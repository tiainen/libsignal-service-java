...
import org.signal.zkgroup.groups.Sho;
...
public final class ServerSecretParams extends ByteArray {
    ...
    byte[] reserved;
    KeyPair authCredentialsKeyPair;
    KeyPair profileKeyCredentialsKeyPair;
    KeyPair sigKeyPair;
    KeyPair receiptCredentialsKeyPair;
    KeyPair pniCredentialsKeyPair;
    
    public ServerSecretParams() {
        super(new byte[0]);
    }
  ...
  public static ServerSecretParams generate(SecureRandom secureRandom) {
    ...
    Sho sho = new Sho("Signal_ZKGroup_20200424_Random_ServerSecretParams_Generate".getBytes());
