      ...
      try {
          if (envelope.hasLegacyMessage()) {
              ...
              SignalServiceContentProto contentProto = SignalServiceContentProto.newBuilder()
                      ...
                      .build();
              return SignalServiceContent.createFromProto(contentProto);
//          System.err.println("SignalServiceCipher: plaintext size = "+plaintext.getData().length);
//        Content   message   = Content.parseFrom(plaintext.getData());
//          System.err.println("SignalServiceCipher: MSG = "+ message.toByteString().toString());
      ...
      SelfSendException
  {
    try {
      ...
      if (envelope.isPreKeySignalMessage()) {
        ...
        metadata       = new SignalServiceMetadata(envelope.getSourceAddress(), 
                envelope.getSourceDevice(), envelope.getTimestamp(),envelope.getServerTimestamp(), false);
        ...
        metadata       = new SignalServiceMetadata(envelope.getSourceAddress(), envelope.getSourceDevice(),
                envelope.getTimestamp(), envelope.getServerTimestamp(), false);
        sessionVersion = sessionCipher.getSessionVersion();
        ...
        SignalServiceAddress  resultAddress       = new SignalServiceAddress(UuidUtil.parse(result.getSenderUuid().get()), result.getSenderE164());
        ...
        metadata       = new SignalServiceMetadata(resultAddress, result.getDeviceId(), 
                envelope.getTimestamp(), envelope.getServerTimestamp(), true);
...
//    }
//      Map<Descriptors.FieldDescriptor, Object> allFields = content.getAllFields();
//      System.err.println("SignalServiceCipher, allfields: "+allFields);
...
//    return new SignalServiceAttachmentPointer(pointer.getCdnId(),
...
//
//        avatar = new SignalServiceAttachmentPointer(pointer.getCdnId(),
//                                                    pointer.getContentType(),
//                                                    pointer.getKey().toByteArray(),
